...
def _grade_mask(algebra: AlgebraContext, grade: int, *, device=None) -> torch.Tensor:
    mask = torch.zeros(algebra.dim, dtype=torch.bool, device=device)
    mask[algebra.layout((grade,)).indices_tensor(device=device)] = True
    return mask


class TestAlgebraContext:
    def test_euclidean_2d_cayley(self):
        # E2: e1*e1=1, e2*e2=1
        # Basis: 1, e1, e2, e12
        # e1*e2 = e12
        # e2*e1 = -e12
        alg = AlgebraContext(p=2, q=0, device="cpu")

        e1 = torch.zeros(1, alg.dim)
        e2 = torch.zeros(1, alg.dim)
        e1[0, 1] = 1.0
        e2[0, 2] = 1.0

        assert torch.allclose(alg.geometric_product(e1, e2)[0, 3], torch.tensor(1.0))
        assert torch.allclose(alg.geometric_product(e2, e1)[0, 3], torch.tensor(-1.0))
    ...
    def test_grade_involution_scalar_unchanged(self):
        alg = AlgebraContext(p=3, q=0, device="cpu")
        mv = torch.zeros(1, 8)
        mv[0, 0] = 3.14
    ...
    def test_clifford_conjugation_sign_pattern(self):
        ...
        for k, expected_sign in enumerate([1, -1, -1, 1]):
            mv = torch.zeros(1, 8)
            mask = _grade_mask(alg, k)
            mv[0, mask] = 1.0
            result = alg.clifford_conjugation(mv)
            assert torch.allclose(result[0, mask], expected_sign * torch.ones(int(mask.sum().item())), atol=1e-6), (
                f"Failed at grade {k}"
            )
    ...
    def test_norm_sq_null_vector_minkowski(self):
        ...
        alg = AlgebraContext(p=2, q=1, device="cpu")
    ...
    def test_dual_grade_shift(self):
        ...
        for k in range(4):
            mv = torch.zeros(1, 8)
            mask_k = _grade_mask(alg, k)
            mv[0, mask_k] = torch.randn(int(mask_k.sum().item()))
            result = alg.dual(mv)
            target_mask = _grade_mask(alg, 3 - k)
