...
def test_algebra_projected_product_matches_small_oracle_and_active_output():
...
def test_bivector_exp_closed_simple_matches_known_basis_formula():
    context = AlgebraContext(3, 0, 0, device=DEVICE, dtype=torch.float64)
    bivector_layout = context.layout((2,))
    ...
    angle = 0.25
    bivectors[0, e12_position] = angle
    actual = context.exp(bivectors, input_layout=bivector_layout, output_layout=rotor_layout)
    expected = torch.zeros_like(actual)
    expected[0, rotor_layout.basis_indices.index(0)] = torch.cos(torch.tensor(angle, dtype=torch.float64))
    expected[0, rotor_layout.basis_indices.index(3)] = torch.sin(torch.tensor(angle, dtype=torch.float64))
...
def test_planner_bivector_exp_executor_matches_between_compact_and_full_output():
    ...
    full = bivector_layout.full(compact)
    ...
    actual_compact = algebra.exp(compact, input_layout=bivector_layout, output_layout=even_layout)
    actual_full = algebra.exp(full)
...
def test_context_declared_product_returns_active_output_without_full_materialization():
    algebra = make_algebra(10, 4, 2, device=DEVICE, dtype=torch.float32)
    vector_layout = algebra.layout((1,))
    left = torch.zeros(1, vector_layout.dim)
    right = torch.zeros(1, vector_layout.dim)
