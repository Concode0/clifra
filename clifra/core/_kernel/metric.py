...
from clifra.core._kernel.contracts import compact_values
from clifra.core._kernel.energy import (
    lane_distance,
    lane_dot_product,
    lane_energy,
...
def scalar_product(
    ...
    *,
    ...
    """
    return algebra.geometric_product(A, B, left=left_layout, right=right_layout, output=algebra.layout((0,)))


def signature_magnitude(algebra: AlgebraContext, values: torch.Tensor) -> torch.Tensor:
    """Return ``sqrt(abs(<~A A>_0))`` for the signed signature form."""
    return torch.sqrt(torch.abs(signature_norm_squared(algebra, values)))


def signature_distance_like(algebra: AlgebraContext, A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
...
def clifford_conjugate(algebra: AlgebraContext, values: torch.Tensor) -> torch.Tensor:
