...
def normalize_grade_product_op(op: str) -> GradeProductOp:
    ...
    if normalized in _GRADE_PRODUCT_OPS:
        return normalized
    supported = ", ".join(sorted(_GRADE_PRODUCT_OPS))
    raise ValueError(f"Unsupported grade product op {op!r}. Supported ops: {supported}")
...
def operation_coefficient(
    ...
    if op in {"geometric_product", "wedge", "symmetric_product", "left_contraction", "right_contraction"}:
        return sign_ab
